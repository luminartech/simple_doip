...
use simple_doip::messages::{
    ...
    EntityStatusResponse, FurtherActionRequired, Header, NackCode, PayloadType,
...
fn check_bytes(name: &str, bytes: &[u8]) {
    ...
    let expected = fs::read_to_string(&path).unwrap_or_else(|e| {
        panic!("{name}: missing fixture {}: {e}; capture with GOLDEN_WRITE=1 on pre-migration main", path.display())
    });
...
#[test]
fn golden_header() {
    check("header_nack", &Header::new(ProtocolVersion::V2012, PayloadType::NegativeAcknowledge, 1));
    check("header_diag_9", &Header::new(ProtocolVersion::V2012, PayloadType::DiagnosticMessage, 9));
    check("header_vid_request_2010", &Header::new(ProtocolVersion::V2010, PayloadType::VehicleIdentificationRequest, 0));
}
...
fn golden_alive_check_response() {
    check("alive_check_response_0000", &AliveCheckResponse { source_address: LogicalAddress(0x0000) });
    check("alive_check_response_0e00", &AliveCheckResponse { source_address: LogicalAddress(0x0E00) });
    check("alive_check_response_ffff", &AliveCheckResponse { source_address: LogicalAddress(0xFFFF) });
}
...
fn golden_power_mode_code() {
    ...
    check("power_mode_not_supported", &DiagnosticPowerModeCode::NotSupported);
}

#[test]
fn golden_entity_status_response() {
    check("entity_status_gateway", &EntityStatusResponse {
        node_type: EntityStatusNodeType::DoIPGateway,
        max_concurrent_tcp_sockets: 4,
        open_tcp_sockets: 0,
        max_data_size: 0x0000_FFFF,
    });
    check("entity_status_node", &EntityStatusResponse {
        node_type: EntityStatusNodeType::DoIPNode,
        max_concurrent_tcp_sockets: 1,
        open_tcp_sockets: 1,
        max_data_size: 64,
    });
    check("entity_status_max", &EntityStatusResponse {
        node_type: EntityStatusNodeType::DoIPNode,
...
fn golden_diagnostic_message() {
    check("diag_msg_empty", &DiagnosticMessage {
        source_address: LogicalAddress(0x0E00),
        target_address: LogicalAddress(0x1000),
        user_data: &[][..],
    });
    check("diag_msg_session_control", &DiagnosticMessage {
        source_address: LogicalAddress(0x0E00),
        target_address: LogicalAddress(0x1000),
        user_data: &[0x10u8, 0x02][..],
    });
    check("diag_msg_16_bytes", &DiagnosticMessage {
        source_address: LogicalAddress(0xE400),
        target_address: LogicalAddress(0x00FF),
        user_data: &[0x22u8, 0xF1, 0x90, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C][..],
    });
}

#[test]
fn golden_diagnostic_message_ack() {
    check("diag_ack_positive_empty", &DiagnosticMessageAck {
    ...
    check("diag_ack_out_of_memory", &DiagnosticMessageAck {
        source_address: LogicalAddress(0x1000),
        target_address: LogicalAddress(0x0E00),
        ack_code: DiagnosticAckCode::OutOfMemory,
        previous_message_data: &[0x10u8, 0x02][..],
    });
    check("diag_ack_transport_error", &DiagnosticMessageAck {
...
fn golden_routing_activation_request() {
    check("routing_req_default_no_tail", &RoutingActivationRequest {
        source_address: LogicalAddress(0x0E00),
        activation_type: ActivationTypeCode::Default,
        reserved: [0, 0, 0, 0],
        reserved_vehicle_manufacturer: None,
    });
    check("routing_req_default_with_tail", &RoutingActivationRequest {
        source_address: LogicalAddress(0x0E00),
        activation_type: ActivationTypeCode::Default,
        reserved: [0, 0, 0, 0],
        reserved_vehicle_manufacturer: Some([0xDE, 0xAD, 0xBE, 0xEF]),
    });
    check("routing_req_central_security", &RoutingActivationRequest {
        source_address: LogicalAddress(0xE400),
...
fn golden_routing_activation_response() {
    check("routing_resp_success_no_oem", &RoutingActivationResponse {
        logical_address_tester: LogicalAddress(0x0E00),
        logical_address_of_doip_entity: LogicalAddress(0x1000),
        routing_activation_response_code: RoutingActivationResponseCode::RoutingSuccessfullyActivated,
        reserved_oem: [0, 0, 0, 0],
        oem_specific: None,
    });
    check("routing_resp_success_with_oem", &RoutingActivationResponse {
        logical_address_tester: LogicalAddress(0x0E00),
        logical_address_of_doip_entity: LogicalAddress(0x1000),
        routing_activation_response_code: RoutingActivationResponseCode::RoutingSuccessfullyActivated,
        reserved_oem: [0, 0, 0, 0],
    ...
    check("routing_resp_denied_unknown_sa", &RoutingActivationResponse {
        logical_address_tester: LogicalAddress(0x0E00),
        logical_address_of_doip_entity: LogicalAddress(0x1000),
        routing_activation_response_code: RoutingActivationResponseCode::DeniedUnknownSourceAddress,
...
fn golden_vehicle_identification_response() {
    check("vid_resp_with_group_id", &VehicleIdentificationResponse {
        vin: [0x41; 17],
    ...
    check("vid_resp_no_group_id", &VehicleIdentificationResponse {
    ...
    check("vid_resp_further_action", &VehicleIdentificationResponse {
        vin: [0x30; 17],
        logical_address: LogicalAddress(0xE400),
        entity_id: [0, 0, 0, 0, 0, 1],
        group_id: Some([1, 1, 1, 1, 1, 1]),
        further_action: FurtherActionRequired::RoutingActivationRequiredToInitiateCentralSecurity,
...
fn golden_full_frames() {
    check_frame("frame_nack", ProtocolVersion::V2012, PayloadType::NegativeAcknowledge, &NackCode::IncorrectPatternFormat);
    check_frame(
        "frame_diag_msg",
